...
import (
	...
	"github.com/sre-project/microservices/internal/product"
...
func main() {
	...
	m := metrics.New("product-service")
	repo := product.NewRepository(pool)

	// gRPC
	grpcSrv := grpc.NewServer()
	pb.RegisterProductServiceServer(grpcSrv, product.NewGRPCServer(repo, m, log))
	...
	go func() {
		log.Info("product-service gRPC listening", "port", cfg.GRPCPort)
	...
	product.NewHandler(repo, m, log).Routes(mux)
	...
	go func() {
		log.Info("product-service HTTP listening", "port", cfg.HTTPPort)
